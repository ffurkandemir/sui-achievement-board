  ...
  return useSuiClientQuery(
      ...
      : ({} as any)) as any,
    {
      ...
      select(data) {
        ...
        // If multiple NFTs exist, select the newest one (by version number)
        const sortedObjs = [...data.data].sort((a, b) => {
          ...
          const versionB = typeof b.data?.version === 'string'
            ...
            : (b.data?.version ?? 0);
          return versionB - versionA; // Newest first (higher version)
        });
        
        // NFT selection - select newest version
...
function App() {
  ...
  // Find user's points in leaderboard
  ...
  // Reserved points sistemi - Read staked and listed points from localStorage
  ...
  // Effective points: From leaderboard if available, otherwise from NFT, THEN subtract reserved
  ...
  async function handleInitAchievement() {
    ...
    if (!isOwner) {
      setError('This board does not belong to you, only the owner can mint.');
      return;
    }
    setError(null);
    setTxDigest(null);
  ...
  async function handleCompleteTask(index: number) {
    ...
    if (!achievement) {
      setError('Please mint an NFT first.');
      return;
    }
    setError(null);
    setTxDigest(null);
  ...
  async function handleClaimDailyReward() {
    ...
    if (!canClaimToday) {
      setError('You already claimed today reward! Try again tomorrow. 🕐');
      return;
    }
    setError(null);
    setTxDigest(null);
      ...
      },
      {
        ...
        onError(err) {
          ...
          if (errStr.includes('MoveAbort') && errStr.includes(', 4)')) {
            setError('❌ You already claimed today reward! You can claim once every 24 hours.');
          } else {
            setError(errStr);
          }
          setIsClaimingDaily(false);
  ...
  async function handleShare() {
    if (!effectiveOwner) {
      setError('Please own or connect to a board first.');
      return;
    }
    if (typeof window === 'undefined') {
      setError('Cannot create link in this environment.');
      return;
    }
    ...
    >
      ...
      >
        ...
        {effectiveOwner && !isOwner && (
          ...
          >
            This board is in read-only mode. Only the
            board owner can complete tasks.
          </div>
        )}
        ...
        {effectiveOwner && account && !achievement && !achievementLoading && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16, background: '#1f2937', borderRadius: 12, border: '1px solid #374151' }}>
            <div style={{ fontSize: 14, color: '#fbbf24' }}>
              ⚠️ <strong>New Contract Version</strong>
            </div>
            <div style={{ fontSize: 13, color: '#d1d5db', lineHeight: 1.5 }}>
              Due to contract update, you need to mint a new Achievement NFT.
              After minting, all features (Governance, Staking, Marketplace) will be available.
            </div>
            {isOwner ? (
              <button
              ...
              <p style={{ fontSize: 13, color: '#9ca3af' }}>No Achievement NFT at this address yet.</p>
            )}
          </div>
        )}
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 12, opacity: 0.7, marginTop: 2 }}>
                    Log in daily and earn bonus points!
                  </div>
                </div>
                {isOwner && (
                  <button
              ...
              {!canClaimToday && isOwner && (
                <div style={{ 
                  ...
                  ✅ You claimed today reward! Come back tomorrow.
                </div>
              )}
              {canClaimToday && isOwner && (
                <div style={{ 
                  ...
                  ⏰ Your daily reward is ready Click to claim bonus points.
                </div>
              )}
            </div>
        ...
        >
          ...
          {eventsLoading && (
            <p style={{ fontSize: 12, opacity: 0.8 }}>Loading events...</p>
          )}
        ...
        {achievement && activeTab === 'governance' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: '#111827', padding: 16, borderRadius: 12, border: '1px solid #1f2937' }}>
              ...
              <div style={{ fontSize: 13, marginBottom: 12, padding: 10, background: '#020617', borderRadius: 6 }}>
                <div style={{ marginBottom: 8 }}>
                  <strong style={{ color: '#10b981' }}>📊 Your Current Points:</strong> <span style={{ fontSize: 16, fontWeight: 'bold', color: '#fbbf24' }}>{effectivePoints}</span>
                </div>
                <div style={{ opacity: 0.8, lineHeight: 1.6 }}>
                  💰 Earn 5% daily by staking<br/>
                  ⚠️ Minimum stake amount: 10 points<br/>
                  🔒 Staked points are virtually locked<br/>
                  <span style={{ fontSize: 11, color: '#fbbf24', marginTop: 4, display: 'block' }}>
                    ⚡ Note: Reserved points tracking is stored in your browser
                  </span>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ fontSize: 13, color: '#9ca3af', fontWeight: 500 }}>💰 Stake Amount (points)</label>
                  <input
                    type="number"
                    value={stakeAmount}
                ...
                <button
                  onClick={async () => {
                    ...
                    if (effectivePoints < amount) {
                      setError(`❌ Insufficient points! Current: ${effectivePoints}, Required: ${amount}`);
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(17,24,39,0.9), rgba(31,41,55,0.9))', padding: 20, borderRadius: 16, border: '1px solid rgba(99,102,241,0.3)', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ fontSize: 13, color: '#9ca3af', fontWeight: 500 }}>📝 Title</label>
                  <input
                    type="text"
                    value={proposalTitle}
                    onChange={(e) => setProposalTitle(e.target.value)}
                    placeholder="e.g.: New Daily Challenge Task"
                    style={{
                      padding: '12px 16px',
                      borderRadius: 10,
                      border: '1px solid rgba(96,165,250,0.3)',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ fontSize: 13, color: '#9ca3af', fontWeight: 500 }}>💬 Description</label>
                  <textarea
                  ...
                  <button
                    onClick={async () => {
                      ...
                      if (effectivePoints < 10) {
                        setError(`❌ You need at least 10 points to create a proposal! Current: ${effectivePoints}`);
                        console.log('%c❌ Insufficient Points', 'color: #ef4444; font-weight: bold;', `Have: ${effectivePoints}, Need: 10`);
                        return;
                      }
                      if (!proposalTitle.trim() || !proposalDescription.trim()) {
                        setError('❌ Title and description fields are required!');
                        return;
                      }
                      ...
                      try {
                        ...
                        if (errStr.includes('InsufficientPoints')) {
                          setError('❌ At least 50 points required to create a proposal!');
                        } else if (errStr.includes('MoveAbort')) {
                          const match = errStr.match(/MoveAbort.*?, (\d+)\)/);
                          const code = match ? match[1] : 'unknown';
        ...
        {achievement && activeTab === 'marketplace' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: '#111827', padding: 16, borderRadius: 12, border: '1px solid #1f2937' }}>
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ fontSize: 13, color: '#9ca3af', fontWeight: 500 }}>💸 Amount to Sell (points)</label>
                  <input
                    type="number"
                    value={sellAmount}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ fontSize: 13, color: '#9ca3af', fontWeight: 500 }}>💵 Price (SUI tokens)</label>
                  <input
                    type="number"
                    value={sellPrice}
                ...
                <button
                  onClick={async () => {
                    ...
                    if (effectivePoints < listAmount) {
                      setError(`❌ Insufficient points! Current: ${effectivePoints}, Amount to sell: ${listAmount}`);
                    ...
                    if (isNaN(suiAmount) || suiAmount <= 0) {
                      setError('❌ Enter a valid SUI price! (e.g.: 0.001)');
                      return;
                    }
            ...
            <div style={{ fontSize: 13, opacity: 0.7, padding: 12, background: '#020617', borderRadius: 8 }}>
              💡 <strong>How it works?</strong> You can sell your points for SUI tokens and buy points from other players with SUI. All transactions are secured by smart contracts on the Sui blockchain.
            </div>
        ...
        {!achievement && !effectiveOwner && (
          <p>
            Connect your wallet and mint your own Achievement NFT to get started.
          </p>
        )}
